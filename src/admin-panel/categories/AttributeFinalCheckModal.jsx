...
const AttributeFinalCheckModal = (props) => {
  ...
  useEffect(() => {
    fetch(
      `${process.env.REACT_APP_URL}/v1/attributes/get?page=1&limit=500&filter[_id][$in]=${selectedAttributeJSON}`
    )
      .then((res) => res.json())
      .then((data) => {
  ...
  useEffect(() => {
    fetch(
      `${process.env.REACT_APP_URL}/v1/category-attributes/get?page=1&limit=500&filter[category_id][$eq]=${id}&filter[attribute_id][$in]=${selectedAttributeJSON}`
    )
      .then((res) => res.json())
      .then((data) => {
  ...
  const updateData = (url, requestBody) => {
    fetch(url, requestBody)
      ...
      .then((data) => {
        if (data) {
          console.log(data);
          navigate("/category/childcategory");
          onClose();
        }
      })
  ...
  const handleSubmit = (e) => {
    ...
    console.log(data);
    updateData(
      `${process.env.REACT_APP_URL}/v1/category-attributes/add-many`,
      requestOptions
    );
  };
