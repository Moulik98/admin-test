...
const StaffModal = ({ id, visible, onClose, modalName }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    const apiEndpoint =
      modalName === "edit"
        ? `${process.env.REACT_APP_URL}/v1/designaiton/editDesignation/${id}`
    ...
    try {
      ...
      if (response.ok) {
        toast.success("Designation added successfully!", data.message);
        onClose()
      ...
      const fetchData = async () => {
        try {
          ...
          if (response.ok) {
            ...
            const { designation_name, designation_short } = data;
